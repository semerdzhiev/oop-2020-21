...
class ChessGameUI {
public:
	virtual void onGameStart(const ChessBoard & board) = 0;
	virtual Move getNextMove() = 0;
	virtual void onCorrectMove(const Move & move, const ChessBoard & board) = 0;
	virtual void onWrongMove(string reason) = 0;
	virtual void onGameEnd() = 0;
};


// Composite Pattern
class ChessGameUIWithLogging : public ChessGameUI {
public:
	ChessGameUIWithLogging(ChessGameUI & ui, ChessGameUI & logger) : ui(ui), logger(logger) {

	}

	virtual void onGameStart(const ChessBoard & board) {
		ui.onGameStart(board);
		logger.onGameStart(board);
	}
	...
	virtual void onCorrectMove(const Move & move, const ChessBoard & board) {
		ui.onCorrectMove(move, board);
		logger.onCorrectMove(move, board);
	}

	virtual void onWrongMove(string reason) {
		ui.onWrongMove(reason);
		logger.onWrongMove(reason);
	}
...
public: 
	...
	void start(ChessGameUI & ui) {
		// ChessGameUI & ui = ServiceDiscovery.getInstance().getChessUI();
		
		ui.onGameStart(this->board);

		while (!this->hasGameEnded()) {
			Move nextMove = ui.getNextMove();

			if (this->makeMove(nextMove)) {
				ui.onCorrectMove(nextMove, this->board);
			} else {
				ui.onWrongMove("Some sort of error!");
			}
		}
...
class ChessGameLoggingUI : public ChessGameUI {
	virtual void onGameStart(const ChessBoard & board) {
	}
	...
	virtual void onCorrectMove(const Move & move, const ChessBoard & board) {
		cout << "LOGGER: ";
		cout << move.getFromPosition().getColumn() << " " << move.getFromPosition().getRow()  << " to " 
		 	 << move.getToPosition().getColumn() << " " << move.getToPosition().getRow()  << endl;
	}
...
class ChessGameConsoleUI : public ChessGameUI {
public:
	virtual void onGameStart(const ChessBoard & board) {
	...
	virtual Move getNextMove() {
		...
		return Move(Position(fromColumn, fromRow), Position(toColumn, toRow));
	}
	...
	virtual void onWrongMove(string reason) {
		cout << "Wrong move!!! " << reason << endl;
	}

	virtual void onGameEnd() {
		cout << "End of the game. The winner is ... " << endl;
	}
...
int main() {
	...
	ChessGameLoggingUI logger;
	ChessGameUIWithLogging uiWithLogging(ui, logger);

	ChessGame game;
	game.start(uiWithLogging);
	// ui.startGame();
